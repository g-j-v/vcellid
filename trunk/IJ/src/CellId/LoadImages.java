...
public class LoadImages extends ij.plugin.frame.PlugInFrame implements
	...
	public void actionPerformed(ActionEvent e) {
		...
		buttonsBox.setPreferredSize(new Dimension(280, 80));
		...
		channelBox.setPreferredSize(hboxSize);
		timeBox.setPreferredSize(hboxSize);
		positionBox.setPreferredSize(hboxSize);
				...
				if (jTree1.getLastSelectedPathComponent() != null) {
					...
					if (node.isLeaf()) {
						...
						previousNode = (DefaultMutableTreeNode) previousNode
								.getChildAt(timeIndex).getChildAt(leafIndex);
						...
						previousNode = (DefaultMutableTreeNode) node
								.getLastChild();
					} else {
						return;
					}
					if (previousNode != null) {
						TreePath path = new TreePath(((DefaultTreeModel) jTree1
								.getModel()).getPathToRoot(previousNode));
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (node == null) {
					...
					if (previousNode == null) {
						...
						.contains("time")) {
					previousNode = (DefaultMutableTreeNode) node
							.getPreviousSibling().getLastChild();
				} else if (node.getUserObject().toString().toLowerCase()
						.contains("position")) {
					previousNode = (DefaultMutableTreeNode) ((DefaultMutableTreeNode) node
							.getLastChild()).getLastChild();
				...
				System.out.println(file.getAbsolutePath() + BAR
						+ previousNode.getUserObject().toString());
				IJ.openImage(file.getAbsolutePath() + BAR
						+ previousNode.getUserObject().toString());
			}
...
//				jTree1.collapsePath(path);
		...
		nextButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				TreePath path = new TreePath(((DefaultTreeModel) jTree1
						.getModel()).getPathToRoot(nextNode));
				updateDisplayRanges(node);
				jTree1.setSelectionPath(path);
				jTree1.scrollPathToVisible(path);
				System.out.println(file.getAbsolutePath() + BAR
		...
		nextTimeButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (node == null) {
					...
					nextNode = (DefaultMutableTreeNode) nextNode
							.getChildAt(position);
				} else if (node.getUserObject().toString().toLowerCase()
						.contains("time")) {
					nextNode = node.getNextSibling();
		...
		nextPositionButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if (node == null) {
					...
					if (nextNode == null) {
						nextNode = (DefaultMutableTreeNode) positionParent
								.getParent().getChildAt(0);
					}
					nextNode = (DefaultMutableTreeNode) nextNode.getChildAt(
