...
public class Segmentation extends ij.plugin.frame.PlugInFrame {
	 ...
	 * 
	 * @param tree
	 *            where to run, depending on selected node.
	 * @param file
	 *            directory where the images can be found.
	 * @param BfType
	 *            indicates if its a normal run or a BF run.
	 * @param test
	 *            indicates if it is a test.
	 */
	public Segmentation(JTree tree, File file,
			final List<String> fluorChannels, final boolean BfType,
			final boolean test) {
		...
		SegmentationValues segmentationValues = SegmentationValues
				.getInstance();
		...
		backgroundRejectSpinner.setModel(new SpinnerNumberModel(5.0, 0.10,
				10.0, 0.1));
		backgroundRejectSpinner.setValue(segmentationValues
				.getBackgroundRejectValue());
		panel.add(backgroundRejectSpinner);

		trackingComparisonSpinner.setBounds(233, 203, 80, 18);
		trackingComparisonSpinner.setModel(new SpinnerNumberModel(5.0, 0.0,
				10.0, 0.1));
		trackingComparisonSpinner.setValue(segmentationValues
				.getTrackingComparisonValue());
		...
		case 1:
			frameAlignmentButtonGroup.setSelected(
					noFrameAlignmentRadioButton.getModel(), true);
			break;
		case 2:
			frameAlignmentButtonGroup.setSelected(
					alignToFirstRadioButton.getModel(), true);
			break;
		case 3:
			frameAlignmentButtonGroup.setSelected(
					alignToBFRadioButton.getModel(), true);
			break;
		default:
			frameAlignmentButtonGroup.setSelected(
					noFrameAlignmentRadioButton.getModel(), true);
		...
		case 1:
			cellAlignmentButtonGroup.setSelected(
					noCellAlignmentRadioButton.getModel(), true);
			break;
		case 2:
			cellAlignmentButtonGroup.setSelected(
					alignIndividualRadioButton.getModel(), true);
			break;
		default:
			cellAlignmentButtonGroup.setSelected(
					noCellAlignmentRadioButton.getModel(), true);
		...
		parameters
				.setToolTipText("Do not use, unless you know what you are doing");
		...
		okButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if (SegmentationValues.getInstance().isNucleusFromChannel() && !fluorChannels.contains(SegmentationValues.getInstance()
						.getNucleusChannel())) {
					new ErrorWindow("<html>El canal seleccionado para núcleo <br/> no es válido.</html>");
				} else {
	...
	public static double getMaxSplitOverMinor() {
		return ((SpinnerNumberModel) maxSplitSpinner.getModel()).getNumber()
				.doubleValue();
	}

	public static double maxDistOverWaist() {
		return ((SpinnerNumberModel) maxDistSpinner.getModel()).getNumber()
				.doubleValue();
	}
	...
	public static double backgroundRejectFactor() {
		return ((SpinnerNumberModel) backgroundRejectSpinner.getModel())
				.getNumber().doubleValue();
	}

	public static double trackingComparison() {
		return ((SpinnerNumberModel) trackingComparisonSpinner.getModel())
				.getNumber().doubleValue();
	}
	...
	private void saveValues() {
		...
		segmentationValues.setAdvancedParametersEnabled(includeParameters
				.isSelected());

		if (frameAlignmentButtonGroup.getSelection().equals(
				noFrameAlignmentRadioButton.getModel())) {
			segmentationValues.setFrameAlignmentButtonSelected(1);
		} else if (frameAlignmentButtonGroup.getSelection().equals(
				alignToFirstRadioButton.getModel())) {
			segmentationValues.setFrameAlignmentButtonSelected(2);
		} else if (frameAlignmentButtonGroup.getSelection().equals(
				alignToBFRadioButton.getModel())) {
			segmentationValues.setFrameAlignmentButtonSelected(3);
		}
		if (cellAlignmentButtonGroup.getSelection().equals(
				noCellAlignmentRadioButton.getModel())) {
			segmentationValues.setCellAlignmentButtonSelected(1);
		} else if (cellAlignmentButtonGroup.getSelection().equals(
				alignIndividualRadioButton.getModel())) {
			segmentationValues.setCellAlignmentButtonSelected(2);
		}

		segmentationValues.setMaxDistValue((Double) maxDistSpinner.getValue());
		segmentationValues
				.setMaxSplitValue((Double) maxSplitSpinner.getValue());
		segmentationValues.setMinPixelsValue((Double) minPixelsSpinner
				.getValue());
		segmentationValues.setMaxPixelsValue((Double) maxPixelsSpinner
				.getValue());
		segmentationValues
				.setTrackingComparisonValue((Double) trackingComparisonSpinner
						.getValue());
		segmentationValues
				.setBackgroundRejectValue((Double) backgroundRejectSpinner
						.getValue());
	}
